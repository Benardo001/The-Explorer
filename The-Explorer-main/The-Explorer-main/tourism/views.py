...
from .forms import BeachForm, BookingForm, ReviewForm, SignupForm, AttractionForm, AccommodationForm, TransportForm, ContactForm, FeedbackForm
...
def contact(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            # Send notification email if recipients configured
            subject = f"Contact message: {form.cleaned_data.get('subject', 'No subject')}"
            body = f"From: {form.cleaned_data.get('name')} <{form.cleaned_data.get('email')}>\n\n{form.cleaned_data.get('message')}"
            recipients = [addr for _, addr in getattr(settings, 'ADMINS', [])]
            if not recipients and getattr(settings, 'EMAIL_HOST_USER', None):
                recipients = [settings.EMAIL_HOST_USER]
            try:
                if recipients:
                    send_mail(subject, body, getattr(settings, 'DEFAULT_FROM_EMAIL', 'no-reply@coastexplorer.local'), recipients, fail_silently=True)
            except Exception:
                pass
            messages.success(request, 'Your message was sent successfully. We will get back to you soon.')
            # If AJAX request, return JSON so frontend can handle without reload
            if request.headers.get('x-requested-with') == 'XMLHttpRequest':
                return JsonResponse({'status': 'ok', 'message': 'Your message was sent successfully.'})
